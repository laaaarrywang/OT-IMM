...
def create_checkerboard_mask(C, H, W, white_first=True):
    ...
    The same spatial pattern is applied to all channels."""
    import torch
    mask = torch.zeros(C, H, W)
    for i in range(H):
        for j in range(W):
            if ((i + j) % 2 == 0) == white_first:
                mask[:, i, j] = 1.0  # Apply to all channels
    return mask.flatten()
...
class ImageAffineCouplingLayer(nn.Module):
    ...
    def __init__(self, C, H, W, mask_flat, time_embed_dim=128,
        ...
        # In nflows: identity_features are where mask <= 0.5
        # In nflows: transform_features are where mask > 0.5
        # transform_net receives identity_features as input!
        identity_indices = torch.nonzero(~mask_bool, as_tuple=False).view(-1)  # mask=0: pass through AND inputs!
...
class TimeIndexedRealNVP(nn.Module):
    ...
    def _init_image_mode(self, shape, num_layers, time_embed_dim,
        ...
        for i, mask in enumerate(masks):
            ...
            class LayerFactory:
                def __init__(self, mask_val, C, H, W, time_embed_dim, hidden_channels, conv_num_blocks):
                ...
                def __call__(self, in_features, out_features):
...
def create_imagenet_flow(**kwargs):
    """
    Create RealNVP flow for ImageNet (3×224×224 images).
    ...
    defaults = dict(
        ...
        conv_num_blocks=4  # Deeper conv networks
    )
    defaults.update(kwargs)
    return TimeIndexedRealNVP(**defaults)